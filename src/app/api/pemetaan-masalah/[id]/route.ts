...
import { IPemetaanMasalah } from "@/lib/interfaces/pemetaan-masalah.interface";

const service = new PemetaanMasalahService();

type Params = { params: Promise<{ id: string }> };

export async function GET(_: NextRequest, { params }: Params): Promise<NextResponse<ApiResponse<IPemetaanMasalah>>> {
  try {
...
export async function PUT(request: NextRequest, { params }: Params): Promise<NextResponse<ApiResponse<IPemetaanMasalah>>> {
  try {
    const { id } = await params;
